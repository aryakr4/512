...
class Board(GameElement):
    ...
    def in_bounds(self, pos: Vec) -> bool:
        ...
        if pos.x < 0 or pos.x > self.rows - 1:
            return False
    ...
    def _move_tile(self, old_pos: Vec, new_pos: Vec):
        ...
        self.tiles[new_pos.x][new_pos.y] = t

    def right(self):
        dir = Vec(0, 1)
        for i in reversed(range(len(self.tiles))):
            for e in reversed(range(len(self.tiles[0]))):
                self.slide(Vec(i, e), dir)

    def left(self):
        dir = Vec(0, -1)
        for i in range(len(self.tiles)):
            for e in range(len(self.tiles[0])):
                self.slide(Vec(i, e), dir)
